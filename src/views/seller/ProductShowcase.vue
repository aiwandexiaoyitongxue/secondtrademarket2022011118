    ...
    <el-card class="search-card">
      ...
      <div class="status-filter" v-if="!hideStatusFilter">
        ...
        <el-tag 
          ...
          class="status-tag" 
          :effect="searchForm.status === 4 ? 'dark' : 'plain'"
        >
          未通过 <span class="tag-count">({{ getStatusCount(4) }})</span>
        </el-tag>
      </div>
    </el-card>
...
import { getProductList, updateProduct, deleteProduct, updateProductStatus } from '@/api/product'
...
const loadProductList = async () => {
  ...
  try {
    ...
    searchForm.merchantId = merchantId
    ...
    if (res.code === 200 && res.data) {
      ...
      productList.value = (records || []).map(processProductData)
...
const handleProductClick = (product) => {
  router.push({
    ...
    query: { id: product.id, readonly: true }
  })
}
...
const loadAllProducts = async () => {
  try {
    ...
    const res = await getProductList(params)
    console.log('获取所有商品响应:', res)
    
    if (res.code === 200 && res.data) {
      // 正确处理返回的数据结构
      const { records } = res.data
      allProducts.value = (records || []).map(processProductData)
    } else {
      allProducts.value = []
    }
  } catch (error) {
    console.error('获取商品统计失败:', error)
    ElMessage.error('获取商品统计失败，请检查网络连接')
  }
}
// 获取状态统计
const getTotalCount = () => {
...
.status-4 {
  background-color: #F56C6C; /* 未通过 - 红色 */
}
