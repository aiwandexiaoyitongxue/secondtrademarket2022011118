...
// 修改用户信息
@Update("UPDATE user SET username=#{username}, real_name=#{realName}, phone=#{phone}, email=#{email}, city=#{city}, gender=#{gender}, role=#{role}, status=#{status} WHERE id=#{id}")
...
/**
   ...
   List<Order> getUserOrdersByStatus(@Param("userId") Long userId, @Param("status") String status);
    ...
    BigDecimal getWalletBalanceByUserId(Long userId);
