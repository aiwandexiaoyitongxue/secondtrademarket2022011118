...
public class UserServiceImpl extends ServiceImpl<UserDao, User> implements UserService {
    ...
    @Autowired
    public UserServiceImpl(UserDao userDao, PasswordEncoder passwordEncoder, UserAddressDao userAddressDao, OrderDao orderDao, WalletRecordDao walletRecordDao) {
...
public void updateWalletBalance(Long userId, Double balance) {
    ...
    public void addMerchantBalance(Long merchantUserId, Double amount) {
        User merchant = getUserById(merchantUserId);
        if (merchant == null || !merchant.isMerchant()) {
            throw new BusinessException("商家不存在");
        }
        Double newBalance = merchant.getWalletBalance() + amount;
        userDao.updateWalletBalance(merchantUserId, newBalance);
    }
    @Override
@Transactional
...
@Override
public List<WalletRecord> getRechargeRecords(Long userId) {
    // 只查 type=1 的流水
    return walletRecordDao.selectByUserIdAndType(userId, 1);
}
}